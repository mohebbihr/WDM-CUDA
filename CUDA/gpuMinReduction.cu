...
__device__ void min_reduce(double* In, double* Out, double * OutIdx, int n){
...
/*__global__ void min_reduce(int* In, int* Out, int * OutIdx, int n){
  ...
  if (tid == 0){
	 Out[blockIdx.x] = myMin;
	 OutIdx[blockIdx.x] = myMinIdx;
	//if(sdata[0] > sdata[1]) Out[blockIdx.x] = sdata[1];
	//else Out[blockIdx.x] = sdata[0];
  }
  //if (tid ==0)printf("tid: %d , Out[%d]:%d \n", tid, blockIdx.x, Out[blockIdx.x]);
}
*/
